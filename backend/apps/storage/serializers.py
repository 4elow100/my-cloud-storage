...
class FolderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Folder
        fields = ["id", "name", "parent", "children", "owner", "created_at"]

    def validate_name(self, value):
        user = self.context["view"].target_user

        if "/" in value:
            raise serializers.ValidationError(
                "Символ '/' недопустим в имени папки"
            )

        parent = self.initial_data.get("parent")
        if Folder.objects.filter(
            owner=user, name=value, parent=parent
        ).exists():
            raise serializers.ValidationError(
...
class FileSerializer(serializers.ModelSerializer):
    ...
    last_download_at = serializers.DateTimeField(
        format="%d.%m.%Y %H:%M:%S", read_only=True
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance is not None:
            self.fields["original_name"].read_only = False
    ...
    def validate(self, attrs):
        ...
        if folder and folder.owner != request_user and not request_user.is_staff:
            raise serializers.ValidationError(
                "Нельзя использовать чужую папку"
            )

        if folder:
            exists = File.objects.filter(
                owner=user, original_name=original_name, folder=folder
            ).exists()
        else:
            exists = File.objects.filter(
                owner=user, original_name=original_name, folder=None
            ).exists()
    ...
    def create(self, validated_data):
        ...
        file_instance = File.objects.create(
            file=uploaded_file, **validated_data
        )
